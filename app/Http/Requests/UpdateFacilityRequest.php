...
class UpdateFacilityRequest extends FormRequest
{
    ...
    protected function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $validator): void {
            $this->assertFacultyMatchesBuilding($validator);
            $this->assertTypeUnchangedIfHasHistory($validator);
        });
    }

    private function assertFacultyMatchesBuilding(Validator $validator): void
    {
        $buildingId = $this->input('building_id');
    ...
    private function assertTypeUnchangedIfHasHistory(Validator $validator): void
    {
        $facility = $this->route('facility');

        if (! $facility || $validator->errors()->has('type')) {
            return;
        }

        if ($facility->type === $this->input('type')) {
            return;
        }

        $hasHistory = DB::table('reservations')->where('facility_id', $facility->id)->exists()
            || DB::table('damage_reports')->where('facility_id', $facility->id)->exists();

        if ($hasHistory) {
            $validator->errors()->add(
                'type',
                'Tipe tidak dapat diubah karena fasilitas sudah memiliki reservasi atau laporan. Buat fasilitas baru lalu nonaktifkan yang lama.',
            );
        }
    }
}
